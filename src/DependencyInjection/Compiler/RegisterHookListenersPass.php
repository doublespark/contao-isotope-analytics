...
namespace Doublespark\IsotopeAnalyticsBundle\DependencyInjection\Compiler;
...
class RegisterHookListenersPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        $listener = new Definition(GlobalsMapListener::class, [['DSA_HOOKS' => $hooks]]);
        $listener->setPublic(true);
        $listener->addTag('contao.hook', ['hook' => 'initializeSystem', 'priority' => 255]);

        $container->setDefinition('analytics.listener.'.ContainerBuilder::hash($listener), $listener);
    }
    ...
    {
        $hooks = [];
        $serviceIds = $container->findTaggedServiceIds('analytics.hook');
    ...
    private function getMethod(array $attributes, string $class, string $serviceId): string
    {
        ...
        $invalid = sprintf('The analytics.hook definition for service "%s" is invalid. ', $serviceId);
